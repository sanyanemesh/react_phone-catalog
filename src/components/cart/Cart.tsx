...
const CartLink = () => {
  ...
  return (
    <>
      <Link className="actions__item actions-cart CartCount" to="/Cart">
        ...
        {cart.length > 0
          ? <span className="CartCount__count">{cart.length}</span>
          : ''}
      </Link>
    </>
  );
    ...
    <>
      ...
      <div className="Cart">
        <ul className="Cart__List">
          {cart.map(item => (
            <li key={item.good.id} className="Cart__Item">
              <button
                type="button"
                className="Cart__Item-Btn Cart__btn-del"
                aria-label="Remove Button"
                onClick={() => dispatch(deleteFromCart(item.good))}
              />
              <div className="Cart__Photo-Container">
                <img alt="card" src={item.good.imageUrl} className="Cart__Photo" />
              </div>
              <p className="Cart__Title">{item.good.name}</p>
              <button
                type="button"
                className="Cart__Item-Btn Cart__btn-minus"
                aria-label="Minus Button"
              />
              <p className="Cart__quantity">1</p>
              <button
                type="button"
                className="Cart__Item-Btn Cart__btn-plus"
                aria-label="Plus Button"
              />
              <p className="Cart__Price">{item.good.price}</p>
            </li>
          ))}
        </ul>
        <div className="Cart__Total">
          <p className="Cart__Total__price">Total price</p>
          <p className="Cart__Total__quantity">Total for * items</p>
